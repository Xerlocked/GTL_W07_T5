...
void FUpdateLightBufferPass::BakeShadowMap(const std::shared_ptr<FEditorViewportClient>& Viewport)
{
    ...
    for (auto Light : PointLights)
    {
        ...
        for (int Face = 0; Face < 6; ++Face)
        {
            ...
            dsvDesc.ViewDimension = D3D11_RTV_DIMENSION_TEXTURE2DARRAY;
            dsvDesc.Texture2DArray.MipSlice = 0;
            dsvDesc.Texture2DArray.FirstArraySlice = Face;
            dsvDesc.Texture2DArray.ArraySize = 1;

            ID3D11RenderTargetView* FaceRTV = nullptr;
            Graphics->Device->CreateRenderTargetView(PointShadowTex, &dsvDesc, &FaceRTV);
